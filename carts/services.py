...
# Разобрать этот код на атомы что бы я знал все!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
class CartService:
    ...
    def __init__(self, request):
        ...
        # Локальный кэш для предотвращения повторных сетевых 
        # уколов в рамках одного запроса
        self._cached_items = None           

        if request.user.is_authenticated:
            self.cart_key = f"cart:user:{request.user.id}"
        else:
            if not request.session.session_key:
                request.session.create()
            self.cart_key = f"cart:anon:{request.session.session_key}"

    def get_items(self) -> list[dict]:
        """Вытаскивает легкую корзину и обогащает ее БЕЗ 
        избыточных повторных запросов"""
        # Если в рамках одного HTTP-запроса метод уже вызывался — 
        # отдаем данные из локальной памяти
        ...
        # Открываем pipeline для пакетного сбора данных каталога
        pipe = self.redis_client.pipeline()
        for p_id in product_ids:
            # Готовим запросы один за другим.
            # ВАЖНО ключи f"catalog:product:{p_id}" не сохраняются
            # при ответе они нужны только как указатели где искать данные для redis.
            pipe.hmget(f"catalog:product:{p_id}", ["name", "price", "image"])
        # Запускает запросы и получает ответы в пакете разом из redis.
        # Хряняться только списки полей ["name", "price", "image"]
        catalog_data_list = pipe.execute()

        enriched_items = []
        for p_id, catalog_fields in zip(product_ids, catalog_data_list):
            # Количество каждой строчки товара только количество.
            qty = int(raw_cart[p_id])

            # Проверка существования товара в кэше каталога. Если товара нет — игнорируем
            if catalog_fields and catalog_fields[0] is not None:
                price_val = catalog_fields[1]
                # Безопасное приведение цены к int: защищает от пустых строк "" и буквенного мусора
        ...
        # Кеш для повторного использования без лишних операций получения товара.
    ...
    def add_or_update_item(self, product_id: int, quantity: int) -> bool:
        ...
        # product_id в каталоге продукта
        catalog_key = f"catalog:product:{product_id}"

        # Проверка существования товара перед добавлением. Нет в кэше — отказ
        if not self.redis_client.exists(catalog_key):
            return False

        # Атомарный инкремент количества в хэше корзины на уровне ядра Redis
        new_qty = self.redis_client.hincrby(self.cart_key, str(product_id), quantity)

        # Если количество ушло в ноль или минус — полностью удаляем поле из хэша
        if new_qty <= 0:
            self.redis_client.hdel(self.cart_key, str(product_id))

        # Накатываем TTL строго в момент записи реальных данных
        self.redis_client.expire(self.cart_key, self.ttl)
        self._cached_items = None  # Сбрасываем локальный кэш
        return True
    ...
    def total_quantity(self) -> int:
        ...
        items = self.get_items()  # Берет данные из локального кэша _cached_items
    ...
    def clear(self) -> None:
        """Полное удаление ключа корзины клиента из ОЗУ"""
        self.redis_client.delete(self.cart_key)
        self._cached_items = None

    def merge_guest_cart(self, guest_session_key: str) -> None:
        """Слияние гостевой корзины с профильной строго через атомарный PIPELINE"""
        if not guest_session_key:
            return

        guest_key = f"cart:anon:{guest_session_key}"
        # Получаем товары корзины пользователя
        raw_guest_cart = self.redis_client.hgetall(guest_key)

        if not raw_guest_cart:
            return

        # Слияние упаковано в один пакетный pipeline, ноль холостых сетевых уколов
        pipe = self.redis_client.pipeline()     # Создали пакет для сбора запросов.
        for p_id, qty in raw_guest_cart.items():
            pipe.hincrby(self.cart_key, p_id, int(qty))

        # Очищаем гостевую корзину и выставляем TTL основной в этом же пакете
        pipe.delete(guest_key)
        pipe.expire(self.cart_key, self.ttl)
        pipe.execute()  #  Запускаем пакет запросов в socket

        self._cached_items = None  # Сбрасываем локальный кэш

# При слиянии лучше использовать lua, во измежания race condition. Изменинить? 
...
# import json
# from django.core.cache import cache           # модуль посредник для работы с Redis. Работаем не на прямую а через него.
